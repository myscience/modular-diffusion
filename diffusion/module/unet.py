...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        num_bits = default(num_bits, 1)
        out_dim = default(out_dim, channels * num_bits)

        self.channels = channels * num_bits
        ...
        # self.fmri_emb = FeatEmbedder(
        #     patch_len = fmri_patch,
        #     embed_dim = fmri_dim,
        #     stride    = fmri_stride,
        # )
        ...
        self.mid_block3 = ConvTimeRes(mid_dim, mid_dim, ctx_dim = ctx_dim, num_group = num_group)

        # Build the upsampling module
        # NOTE: We need to make rooms for incoming residual connections
    ...
    def forward(
        ...
        # ctrl = self.fmri_emb(fmri) if exists(fmri) else fmri
